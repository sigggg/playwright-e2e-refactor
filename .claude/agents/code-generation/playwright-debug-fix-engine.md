...
#### 1-1. 最新の失敗ログとHTML構造を確認

```bash
# test-results/ ディレクトリから最新の失敗HTMLを取得（優先）
# test-hooks.ts使用時に自動保存される実HTMLを活用
ls -lt test-results/*/failure-page.html 2>/dev/null | head -n 1

# 失敗ログを取得
ls -lt test-results/*/test-failed-*.txt 2>/dev/null | head -n 1
cat <最新の失敗ログパス>

# その他のログ・HTMLファイルも確認
find ./test-results -name "*.txt" -o -name "*.log" -o -name "*.html" 2>/dev/null | head -n 10
```

**抽出情報**:
- **failure-page.html**: テスト失敗時の実際のHTML構造（最優先・証拠として最も信頼性が高い）
...
# 失敗時のHTML構造を保存（test-hooks.ts使用時に存在）
if [ -f test-results/latest/failure-page.html ]; then
  cp test-results/latest/failure-page.html ${DEBUG_DIR}/error_page.html
  echo "✅ failure-page.html を保存しました（test-hooks.ts使用）"
fi
...
// HTML全体を取得
// 注: Phase 1-4 で failure-page.html が既に保存されている場合、
//     そちらを優先的に使用する（実際の失敗時のHTML）
// ここで取得するのは「再現実行時のHTML」（比較用）
