...
ユーザー指示に複数のタイプが含まれる場合（例: 「セレクタを追加して、失敗しているテストも修正して」）、以下の優先順位で処理します:

1. **複合タイプの特定**:
   - 各キーワードを検出し、該当する全タイプをリストアップ
   - 例: 「追加」+「修正」→ 機能追加 + バグ修正

2. **処理順序の決定**（優先度順）:
   - **優先度1**: バグ修正 → まず既存の失敗を解消
   - **優先度2**: 機能追加 → 安定した状態で新機能を追加
   - **優先度3**: 品質改善・セレクタ変更 → 最後に品質向上

3. **逐次実行**:
   - 1つずつ順番に処理し、各タイプの完了後に次へ進む
   - 例: バグ修正 → テスト実行 → 全グリーン確認 → 機能追加 → テスト実行 → レビュー

4. **修正計画への反映**:
   - Phase 0の修正計画で、複合タイプであることとその処理順序を明示
   - ユーザー承認時に処理順序を確認

**複合パターンの例**:

```markdown
ユーザー指示: 「tests/login/login.spec.ts が失敗しているので修正して、ついでにパスワードリセット機能のテストも追加してください」
...
処理順序:
Phase 1-A: バグ修正（Debug Engine起動）
  → Phase 2: テスト実行 → 全グリーン確認
  → Phase 1-B: 機能追加（Code Generator起動）
  → Phase 2: テスト実行
  → Phase 4: コード品質チェック
  → Phase 5: 仕様完全性レビュー（機能追加のため）
  → Phase 6: ドキュメント更新
```
...
- [ ] 実行結果を記録
- [ ] デバッグ再試行カウンタを管理（`debug_retry_count`）
...
**失敗時**:
- デバッグ再試行カウンタ（`debug_retry_count`）をチェック
  - `debug_retry_count < 3` → Phase 3（デバッグ）へ、カウンタを+1
  - `debug_retry_count >= 3` → エスカレーション（デバッグ限界）

**デバッグ再試行カウンタの管理**:
- **初期値**: 0
- **カウント対象**: Phase 2（テスト実行）→ Phase 3（デバッグ）→ Phase 2（再実行）のサイクル回数
- **上限**: 3回（Phase 3を3回実行したら停止）
- **リセット条件**: Phase 2で全テストが成功した場合
...
**実行条件**: Phase 2で失敗 かつ `debug_retry_count < 3`
...
- デバッグ試行回数: {debug_retry_count + 1} / 3回目

【修正履歴】
Phase 1での修正内容:
{Phase 1で行った修正の概要}

【これまでのデバッグ履歴】（2回目以降の場合）
{前回までのデバッグ内容と結果}
```
...
**Phase 3完了後の遷移**:
- **必ず Phase 2へ戻る**: 修正内容をテスト実行で検証
- Phase 2で成功 → Phase 4へ
- Phase 2で失敗 → `debug_retry_count`をチェック
  - `debug_retry_count < 3` → 再度Phase 3へ
  - `debug_retry_count >= 3` → エスカレーション

**ループフロー**:
```
Phase 1（修正実施）
  ↓
Phase 2（テスト実行）→ 成功 → Phase 4へ
  ↓ 失敗（debug_retry_count=0）
Phase 3（デバッグ1回目）
  ↓
Phase 2（テスト再実行）→ 成功 → Phase 4へ
  ↓ 失敗（debug_retry_count=1）
Phase 3（デバッグ2回目）
  ↓
Phase 2（テスト再実行）→ 成功 → Phase 4へ
  ↓ 失敗（debug_retry_count=2）
Phase 3（デバッグ3回目）
  ↓
Phase 2（テスト再実行）→ 成功 → Phase 4へ
  ↓ 失敗（debug_retry_count=3）
エスカレーション（デバッグ限界）
```
...
Phase 6の開始時に、以下のドキュメントファイルの存在を確認します:

```bash
# 修正対象テストのディレクトリ内を確認
ls tests/{service}/README.md 2>/dev/null
ls tests/{service}/TEST_DETAILS.md 2>/dev/null
...
| ファイル | 存在する場合 | 存在しない場合 |
|---------|-------------|---------------|
| **README.md** | 内容を更新 | ⚠️ 警告を出してスキップ（「README.mdが存在しないため、ドキュメント更新をスキップしました」と記録） |
| **TEST_DETAILS.md** | 内容を更新 | ⚠️ 警告を出してスキップ（「TEST_DETAILS.mdが存在しないため、詳細ドキュメント更新をスキップしました」と記録） |
| **CHANGELOG.md** | 内容を更新 | ✅ プロジェクトルートに新規作成し、今回の変更を記録 |

**新規プロジェクト・移行直後の対応**:

README.mdやTEST_DETAILS.mdが存在しない場合（新規移行直後のプロジェクト等）は、以下のように対応します:

1. **Phase 7の総合レポートに記録**:
   - 「ドキュメントファイルが存在しないため、Phase 6をスキップしました」と明記
   - 今回の修正内容は総合レポートに詳細に記載

2. **ユーザーへの推奨事項を提示**:
   ```
   【推奨】README.mdとTEST_DETAILS.mdを作成することで、今後の保守性が向上します。
   - README.md: テスト概要、実行方法、テストケース一覧
   - TEST_DETAILS.md: 各テストの詳細、既知の制約事項、改善案
   ```

**やること**:
- [ ] ドキュメントファイルの存在確認
- [ ] README.md更新（機能追加時のみ、**存在する場合のみ**）:
  - テストケース一覧に新規テストを追加
  - 確認事項の更新
- [ ] TEST_DETAILS.md更新（該当する場合のみ、**存在する場合のみ**）:
  - 新規テストケースの詳細を追加
  - 既知の制約事項・今後の改善案を更新
- [ ] 変更履歴の記録:
  - `CHANGELOG.md` が存在する場合: 内容を更新
  - `CHANGELOG.md` が存在しない場合: 新規作成して今回の変更を記録
  - CHANGELOG.mdも作成できない場合: README.mdの「変更履歴」セクションに記録（README.mdも存在しない場合は総合レポートのみ）
...
### 1. デバッグ限界（Phase 2→3サイクルを3回実行後も失敗）

**条件**: `debug_retry_count >= 3`（Phase 3を3回実行したが、Phase 2で失敗し続けた場合）

```markdown
## 🚨 デバッグ限界に達しました

### 試行内容（Phase 2→3サイクルを3回実行）
1. Phase 3（1回目）: セレクタの修正 → Phase 2で失敗
2. Phase 3（2回目）: 待機戦略の変更 → Phase 2で失敗
3. Phase 3（3回目）: データ形式の調整 → Phase 2で失敗

### デバッグ再試行カウンタ
- debug_retry_count: 3 / 3（上限到達）

### 最終エラー（Phase 2での最終実行結果）
{エラー内容}
